"""
Main CLI application for Linearator.
    ...
    def get_client(self) -> LinearClient:
        ...
        if self.client is None:
            if not self.authenticator or not self.authenticator.is_authenticated:
                console.print(
                    "[red]Error: Not authenticated. Please run 'linearator auth login' first.[/red]"
                )
                sys.exit(1)
...
def main(
    ...
    """
    Linearator - A comprehensive CLI for Linear issue management.

    Manage Linear issues, teams, and projects from the command line with
    powerful filtering, search, and batch operations.

    Examples:
        linearator auth login                    # Authenticate with Linear
        linearator issue list                    # List issues
        linearator issue create "Bug fix"       # Create new issue
        linearator team list                     # List teams
        linearator search "api bug"              # Search issues

    For more information, visit: https://github.com/linearator/linearator
    """
    ...
    try:
        ...
        console.print(f"[red]Error initializing Linearator: {e}[/red]")
        if debug:
...
def version(ctx: click.Context) -> None:
    ...
    console.print(f"Linearator version {__version__}")
...
def status(ctx: click.Context) -> None:
    ...
    if auth_status:
        ...
        console.print(
            "[yellow]Please run 'linearator auth login' to authenticate.[/yellow]"
        )
