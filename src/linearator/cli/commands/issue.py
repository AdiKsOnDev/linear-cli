...
def list(
    ...
    Examples:
        ...
        linearator issue list --team ENG --limit 10
        linearator issue list --assignee john@example.com --state "In Progress"
        linearator issue list --labels "bug,urgent" --priority 4
    """
...
def create(
    ...
    Examples:
        linearator issue create "Fix login bug"
        linearator issue create "New feature" --description "Add user profiles"
        linearator issue create "Bug fix" --team ENG --assignee jane@example.com --priority 3
        linearator issue create "Enhancement" --labels "feature,ui" --description "Improve UX"
    """
    cli_ctx = ctx.obj["cli_context"]
    client = cli_ctx.get_client()
    config = cli_ctx.config
        ...
        if team:
            ...
            if not team_id:
                raise ValueError(
                    ...
                    "Use --team option or set default team with 'linearator team switch'"
                )
...
def show(ctx: click.Context, issue_id: str) -> None:
    ...
    Examples:
        linearator issue show ENG-123
        linearator issue show team_abc123_issue_def456
    """
    cli_ctx = ctx.obj["cli_context"]
    client = cli_ctx.get_client()
    config = cli_ctx.config
...
def update(
    ...
    Examples:
        linearator issue update ENG-123 --title "Updated title"
        linearator issue update ENG-123 --state "Done" --assignee john@example.com
        linearator issue update ENG-123 --priority 4 --labels "bug,critical"
        linearator issue update ENG-123 --description "Updated description"
    """
    cli_ctx = ctx.obj["cli_context"]
    client = cli_ctx.get_client()
    config = cli_ctx.config
...
def delete(ctx: click.Context, issue_id: str, confirm: bool) -> None:
    ...
    Examples:
        linearator issue delete ENG-123
        linearator issue delete ENG-123 --confirm  # Skip confirmation prompt
    """
    cli_ctx = ctx.obj["cli_context"]
    client = cli_ctx.get_client()
    config = cli_ctx.config
