...
def list(
    ...
    \b
    Examples:
        linearator user list                    # List all accessible users
        linearator user list --team ENG         # List users in Engineering team
        linearator user list --format json      # JSON output for scripting
...
def show(
    ...
    \b
    Examples:
        linearator user show john@company.com        # Show user by email
        linearator user show usr_123456789           # Show user by ID
        linearator user show john@company.com --team ENG  # Workload in specific team
...
def workload(
    ...
    \b
    Examples:
        linearator user workload                        # Overall workload analysis
        linearator user workload --team ENG             # Team-specific analysis
        linearator user workload --sort-by high-priority # Sort by high-priority issues
        linearator user workload --limit 20             # Show more users
...
def suggest(
    ...
    \b
    Examples:
        linearator user suggest 5                    # Suggest assignments for 5 issues
        linearator user suggest 3 --team ENG        # Team-specific suggestions
        linearator user suggest 2 --priority 4      # For urgent issues
        linearator user suggest 4 --exclude john@company.com  # Exclude specific user
