...
def interactive(ctx: click.Context) -> None:
    ...
    async def search_builder_workflow() -> None:
        ...
        console.print(
            "[dim]Use 'linearator search' command for actual search execution[/dim]"
        )
    ...
    async def bulk_operations_workflow() -> None:
        ...
        if operation == "update-state":
            ...
            console.print(
                f"linearator bulk update-state -q '{query}' --new-state '{new_state}'"
            )
            ...
            console.print("\n[yellow]Would execute:[/yellow]")
            console.print(
                f"linearator bulk assign -q '{query}' --assignee '{assignee}'"
            )
            ...
            if action == "add":
                console.print(
                    f"linearator bulk label -q '{query}' --add-labels '{labels}'"
                )
            else:
                console.print(
                    f"linearator bulk label -q '{query}' --remove-labels '{labels}'"
                )
    ...
    async def team_management_workflow() -> None:
        ...
        console.print(
            "\n[dim]Use 'linearator team info <key>' for detailed team information[/dim]"
        )
    ...
    async def workload_analysis_workflow() -> None:
        ...
        console.print(
            "\n[dim]Use 'linearator user workload --team <key>' for detailed analysis[/dim]"
        )
